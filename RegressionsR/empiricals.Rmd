...
# use last year retur
  temp = clientdaily[account_date < '2017-12-31' & age > 365,
                   list(account_date = account_date,
                        closing_date = closing_date,
                   ret =  raprofit365,
                   firstobs = firstobs,
...
for(j in 1:3){
  ...
  tempplot = xx[, list(mret = mean(ret, na.rm = T)), by = c('account_date', 'exinextyr')]
  ...
  legend('top', lty = 1, legend = c('Reamin for next year', 'Exit during next year'), col = c('black', 'red'), bty = 'n')
...
gc()
xx = merge(regtable[,c('client','gender', 'nsec', 'mean_chf_tr', 'TradesOptions','enterage')], xx)

memory.limit(size = 1e20)
gc()
time = Sys.time()
gc()
xxplmgender = plm(rreturn365 ~ age + gender, data = xx, index = c('client', 'account_date'), effect = 'time')
Sys.time() - time
