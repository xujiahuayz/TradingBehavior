...
for pi0 in pi0value:
    col = clrmap(norm(pi0))
    plt.plot(tees, [funcpi(tee, pi0) for tee in tees], color = col)
plt.xlabel("Trading age $t$")
...
for pi0 in pi0value:
    ...
    plt.text(mar, 1 - mar, str(
        '$\\pi_0 = ' + str(round(pi0, 2)) + '$'
...
for c in cvalue:
    ...
    if c > 0:
            ...
            for pi0 in pi0value:
                if pi0 <= pi0min(phi, c):
                    ...
                    for tee in tees:
                        ...
                        plt.xlabel("Future trading age $\\tau, t \\leq \\tau \\leq \\tau_E^*(t)$")
                        plt.ylabel("Effort $\\eta(\\tau \\,| \\, t)$")

                    plt.scatter(tees, [eta(tee, tee, phi, pi0, c) for tee in tees],
                                color='red', marker='.', label='$\\eta(t\\,|\\,t)$', zorder=2)
                    plt.scatter(taues, etaes,
                                color='blue', marker='.', label='$\\eta(\\tau_E^*(t) \\,|\\,t)$', zorder=2)
                    axes = plt.gca()
                    axes.set_xlim([0, taumax])
                    axes.set_ylim([0, 1])
                    sm = plt.cm.ScalarMappable(cmap=clrmap, norm=plt.Normalize(vmin=0, vmax=teemax))
        ...
        for phi in phivalue:
            ...
            for pi0 in pi0value:
                if pi0 <= pi0min(phi, c):
                    ...
                    plt.plot(tees, [eta(tee, tee, phi, pi0, c) for tee in tees],
                                color=col, marker='.')
            plt.xlabel("Trading age $t$")
        ...
        for pi0 in pi0value:
            ...
            axes.set_xlim([0, taumax])
            axes.set_ylim([0, 1.2])
            sm = plt.cm.ScalarMappable(cmap=clrmap, norm=plt.Normalize(vmin=1 - phimax, vmax=phimax))
    ...
    for phi in phivalue:
        ...
        for pi0 in pi0value:
            ...
            try:
                ...
                tees = teevalue[teevalue < tauu]
                taueult.extend([tauu])
                plt.plot(tees, [taueroot(tee, pi0, phi, c=c) for tee in tees], color=col, marker='.')
            except:
                break

        plt.xlabel("Trading age $t$")
        plt.ylabel("Optimal exit time $\\tau_E^*$")
        plt.plot(taueult, taueult, color='red', marker='.', label = '$\\tau_E^{\\bar{*}}$')
        axes = plt.gca()
        axes.set_xlim([0, teemax])
    ...
    phimin = 0.01
    phimax = 1.81
    pi0value = np.concatenate([np.linspace(start = 0.9999, stop = 0.9, num = 10),
                               np.linspace(start = 0.91, stop = 0.1, num = 20),
                               np.linspace(start = 0.09, stop = 0.0001, num = 10)], axis=0)
        ...
        if pimi < 0.999:
            ...
            for pi0 in pi0value:
                try:
                    ...
                    next
            plt.plot(pi0value[0:taueult.__len__()], taueult, color=col, marker='.', label=str(np.round(phi, 2)))
        else:
            break
    ...
    for tee in teevalue:
        ...
        plt.plot(phis, [
            pimin0(phi, tee, c) for phi in phis
        ], color=col)
...
for c in cvalue:
    ...
    for rouexit in rouexitvalue:
        ...
        for lbd in lbdvalue:
            ...
            plt.close('all')
            for nu in nuvalue:
                plt.plot(deltats, res[paramlist.index((c, lbd, rouexit, nu))]
                         , label='$' + str(nu) + '$')

            plt.legend(bbox_to_anchor=(1, 1), loc=2, title='$\\nu$', handlelength=0.5)
            plt.text(0.78, 0.27, '$\lambda=' + str(lbd) + '$', transform=plt.gcf().transFigure)
            plt.text(0.5, 0.91, '$\\rho_{exit}=' + str(rouexit) + '$', transform=plt.gcf().transFigure)
