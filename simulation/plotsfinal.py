...
plt.close()
plt.plot(teev, yyall, 'k', label=
'Average return, aggregate: $\\frac{\\int R\\mu^R(R, t) dR}{m(t)}$')
...
plt.tight_layout()
plt.savefig(figdir + 'AvgReturn.pdf')
plt.show()
...
for teevind in [49, 50, 51]:
    ...
    plt.plot(teev[sel], (i*yyg)[sel], label=
             "$\\frac{\\mu_G(t =" + str(temptee) + ")+\\mu_{inf}(t =" + str(temptee) + ")}{m(t =" + str(temptee) + ")} = $" + str(round(i,2)))
plt.xlabel("Trading age $t$")
plt.ylabel("Average return")
axes = plt.gca()
...
plt.legend(loc='lower center',
           handlelength=2, frameon=False, title='Proportion of type $G$')

plt.tight_layout()
plt.savefig(figdir + 'CohortReturn.pdf')
plt.show()
...
plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Trading age $t$',
           handlelength=1, frameon=False)
plt.xlabel("Return $R$")
plt.ylabel(
    "Trading mass $\\frac{\\mu^R(R, t)}{M(t)}$")
plt.tight_layout()
plt.savefig('../figure/MassReturn.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(figdir + 'ExitLikeliGivenAgePy.pdf')
plt.show()
...
plt.close('all')
for i in [0, 20, 40]:
    plt.plot(teev, tempdta['exr' + str(i)]/tempdtaR['R' + str(i)],
             label='$R=' + str(res[i]) + '$')
axes = plt.gca()
axes.set_ylim([0, 0.134])
axes.spines['right'].set_visible(False)
...
plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Trading return $R$',
           handlelength=1, frameon=False)
plt.xlabel("Trading age $t$")
plt.ylabel("Exit likelihood $\\frac{\\mu_{exit}^R(R, t)}{\\mu^R(R, t)}$")
plt.tight_layout()
plt.savefig(figdir + 'ExitLikeliGivenReturnPy.pdf')
plt.show()
...
for i in [6, 50, 64]:
    tv = teev[i]
    plt.plot(res, (np.array(tempdtaR[i:(i + 1)])[0]) / masv[i],
             label='Remain density $\\frac{\\mu^R(R, t=' + str(tv) + ')}{m(t=' + str(tv) + ')}$')
    plt.plot(res, np.array(tempdta[i:(i + 1)])[0] / exitmasv[i],
             label='Exit density $\\frac{\\mu_{exit}^R(R, t=' + str(tv) + ')}{m_{exit}(t=' + str(tv) + ')}$'
             )
    axes = plt.gca()
...
for k in ns:
    ...
    for i in [0, 20, 40]:
        murexitt = (1 - rouexit) * murtexitc(
            R=res[i], Time=Ts[k]) + rouexit * murt(Time=Ts[k], R=res[i])
        plt.plot(teev[sel],  tempdta.iloc[sel, i]/murexitt,
                 label='$R=' +
                       res[i].round(2).__str__() + '$')
    axes = plt.gca()
    # axes.set_xlim([0, 6])
    ...
    plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Return $R$',
               handlelength=1, frameon=False)
    plt.xlabel("Trading age $t$")
    plt.ylabel(
        "Density $\\frac{\\mu_{exit}^R(R,t)}{\\int_0^T \\mu_{exit}^R(R, t) dt}$")
    plt.tight_layout()
...
plt.tight_layout()
plt.savefig(figdir + 'ExitMasPy.pdf')
plt.show()



plt.close()
plt.plot(teev, -(exit_rate_G + exit_rate_B + exit_rate_inf)/masv, 'k', label="Total exit $-\\frac{m'(t)}{m(t)}$")
# plt.plot(teev,  [rouexit] * len(teev), ':', label='Exit due to exogenous shock')
plt.plot(teev, -(exit_rate_G + exit_rate_inf)/masv, '--', label='Exit from type $G$ and $inf$')
...
plt.tight_layout()
plt.savefig(figdir + 'ExitlikeliPy.pdf')
plt.show()
...
for i in [6, 50, 64]:
    ...
    plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Trading age $t=' + str(tv) + '$', handlelength=1)
    plt.xlabel("Return $R$")
    plt.ylabel("Mass")
    plt.show()
...
for phiv in [0.01, 0.05, 0.35]:
    plt.plot(teev, [lstar(tee=i, phi=phiv) for i in teev]
...
plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Running cost $\\varphi$',
...
massinf = np.vectorize(lambda Time: quad(lambda x: muGinf(x), 0, Time,
                epsrel=0.001)[0])(Tvs)
...
plt.tight_layout()
plt.savefig(figdir + 'MassThruTimePy.pdf')
plt.show()



aggr = np.vectorize(lambda Time: quad(lambda x: (muGinf(x) + maspOne(x, True)) * funcf(x), 0, Time,
                epsrel=0.001)[0])(Tvs)
...
plt.tight_layout()
plt.savefig(figdir + 'AvgRThruTimePy.pdf')
plt.show()


aggexit = np.vectorize(
    lambda Time: quad(lambda x: exitpOne(x, True) + exitpOne(x, False),
                      0, Time, epsrel=0.001)[0])(Tvs)
...
plt.tight_layout()
plt.savefig(figdir + 'ExitThruTimePy.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(figdir + 'ExitMassThruTimePy.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(figdir + 'returnexitPy.pdf')
plt.show()
...
plt.plot(teev, masptv, '--', label=
'Type $G$ mass (excl. $inf$)')
plt.plot(teev, maspfv, '-.', label=
'Type $B$ mass')
axes = plt.gca()
axes.set_xlim([0, 19.9])
axes.set_ylim([0, 1])
...
plt.legend(bbox_to_anchor=(1, 1), loc=1,
           handlelength=2, frameon=False)
plt.tight_layout()
plt.savefig(figdir + 'MassByAgePy.pdf')
plt.show()
...
plt.legend(bbox_to_anchor=(1, 1), loc=1, title='Calendar time $T$',
           handlelength=1, frameon=False)
plt.tight_layout()
plt.savefig(figdir + 'AgeDensityGivenTimePy.pdf')
plt.show()
...
for k in ns:
    sel = np.where(teev < Ts[k])[0]
    for i in [0, 20, 40]:
        plt.plot(teev[sel], tempdtaR.iloc[sel, i]/murt(Time=Ts[k], R=res[i]),
...
plt.legend(bbox_to_anchor=(1, 1), loc=1, handlelength=0.5,
               frameon=False)
plt.tight_layout()
plt.savefig('../figure/ldist1.pdf')
plt.show()
